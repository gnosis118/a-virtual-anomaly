...
export const scheduledPosts: ScheduledPost[] = contentCalendarPosts.map((post, index) => {
...
// Helper function to find posts for a specific date
export const getPostsForDate = (date: Date | undefined) => {
  if (!date) return [];
...
// Get all days with scheduled posts for calendar highlighting
export const daysWithPosts = scheduledPosts.map(post => new Date(post.publishDate));
